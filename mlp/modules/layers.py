...
class Linear(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        if self.bias is not None:
            ...
            return input @ np.transpose(self.weight)
...
class BatchNormalization(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        if self.training:
            ...
            self.running_var = (1 - self.momentum) * self.running_var + self.momentum * self.var * input.shape[1] / (input.shape[1] - 1)
        else:
            self.norm_input = (input - self.running_mean) / np.sqrt(self.running_var + self.eps)

        return self.weight * self.norm_input + self.bias if self.affine else self.norm_input
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        :return: array of shape (batch_size, num_features)
        """
        grad_normalized = grad_output * self.weight if self.affine else grad_output
        if not self.training:
            return grad_normalized / np.sqrt(self.running_var + self.eps)

        grad_var = -np.sum(grad_normalized * self.input_mean, axis=0) * self.input_mean / (input.shape[0] * (self.var + self.eps) ** 1.5)
        grad_input_adjusted = grad_normalized * self.inv_sqrt_var + grad_var
        grad_mean = -np.sum(grad_input_adjusted, axis=0) / input.shape[0]
        return grad_input_adjusted + grad_mean
